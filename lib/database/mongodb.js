...
export async function getCollection(collectionName) {
	...
	return {
		...
		find: async (query = {}, options = {}) => {
			...
			return results.map((row) => fromObjectId(row));
		},

		// 分页查询
		findWithPagination: async ({ query = {}, pageIndex = 1, pageSize = 10, sort = { createdAt: -1 } }) => {
			query = processObjectIds(query);
		...
		// 连表分页查询
		async findWithLookup({ query = {}, pageIndex = 1, pageSize = 10, sort = { createdAt: -1 }, lookups = [] }) {
			query = processObjectIds(query);
		...
		// 聚合查询
		aggregate: (pipeline) => {
			// 不处理 pipeline 中的 ObjectId，因为聚合查询有特殊语法
 ...
 * 检查字符串是否是有效的 UUID 格式
 * @param {string} id - 要检查的ID字符串
 * @returns {boolean} 是否是有效的 UUID 格式
 */
