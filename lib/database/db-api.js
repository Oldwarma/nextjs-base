import { connectToDatabase, getCollection, toObjectId, fromObjectId, isValidObjectId } from '@/lib/database/mongodb.js';
		...
		if (Array.isArray(foreignDB) && foreignDB.length > 0) {
			for (const foreign of foreignDB) {
				const {
					...
					convertToObjectId = false, // ✅ 新增：是否需要将 localKey 转换为 ObjectId
				} = foreign;
				...
				if (convertToObjectId) {
					// 需要类型转换：将字符串转为 ObjectId 后再比较
					// 使用 onError 处理无效的 ObjectId 字符串（如 'system', 'admin'）
					matchExpr = {
						$cond: {
										...
										$map: { 
											...
											in: { 
												$convert: { 
													input: '$$item', 
													to: 'objectId', 
													onError: null  // ✅ 转换失败时返回 null，跳过该项
												} 
											} 
										} 
									}
