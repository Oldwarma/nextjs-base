...
export async function uploadToR2({ body, key, contentType, metadata = {} }) {
	...
	try {
		...
		if (error.Code === 'AccessDenied' || error.code === 'AccessDenied') {
			throw new Error('R2 Access Denied: Please check your API token permissions and bucket configuration. Make sure the token has "Object Read & Write" permission for this bucket.');
		}
