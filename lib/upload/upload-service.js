...
import { uploadToR2, generateFileKey, getPublicUrl, deleteFromR2 } from './r2-client';
...
export async function deleteFile(keyOrUrl, userId) {
	try {
		...
		// 查找上传记录
		const uploadsCollection = await getCollection('uploads');
		const record = await uploadsCollection.findOne({ key });
		...
		// 删除数据库记录
		await uploadsCollection.deleteOne({ key });
