...
import { add, remove, getOne, selects } from '@/lib/database/db-api';
 ...
 * @param {Object} options - 额外选项
 * @param {boolean} options.isAdmin - 是否是管理员（管理员可以删除任何文件）
 * @param {boolean} options.skipPermissionCheck - 是否跳过权限检查（慎用）
...
export async function deleteFile(keyOrUrl, userId, options = {}) {
	...
	try {
		...
		// 验证权限（管理员可以删除任何文件，普通用户只能删除自己的文件）
		...
		// 删除数据库记录（使用 db-api 的 remove 方法，根据条件删除）
		await remove({
			dbName: COLLECTION_NAME,
			whereJson: { key },
		});
