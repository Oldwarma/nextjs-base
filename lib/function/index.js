/**
 * NB 公共函数库 (nb.pubfn)
 * 参考 vk-unicloud 的 pubfn API 设计
 * @see https://vkdoc.fsq.pub/client/jsapi.html#common
 * 
 * 使用方式:
...
// 树形结构工具
import tree from './treeUtil.js'

/**
 * 树形结构工具
 * 包含: arrayToTree, treeToArray, findInTree, mapTree, filterTree 等
 * @example
 * nb.pubfn.tree.arrayToTree(list);
 ...
 * nb.pubfn.getDateInfo(new Date());
 */
pubfn.getDateInfo = pubfn.timeUtil.getDateInfo;

/**
 * 日期对象转换（此API已弃用，建议用nb.pubfn.timeFormat代替）
 * @param {Date || Number} date 需要转换的时间
 ...
 * 返回时间戳形式
nb.pubfn.getOffsetTime(new Date(), {
	year:0,
	month:0,
	day:0,
	hours:0,
 ...
 * nb.pubfn.getHourOffsetStartAndEnd(0);
 */
 ...
 * nb.pubfn.getDayOffsetStartAndEnd(0);
 */
 ...
 * nb.pubfn.getWeekOffsetStartAndEnd(0);
 */
 ...
 * @param {Date || Number} date 指定从那天开始计算
 * nb.pubfn.getMonthOffsetStartAndEnd(0);
 */
 ...
 * @param {Date || Number} date 指定从哪个时间节点开始计算
 * nb.pubfn.getQuarterOffsetStartAndEnd(0);
 */
 ...
 * @param {Date || Number} date 指定从那天开始计算
 * nb.pubfn.getYearOffsetStartAndEnd(0);
 */
...
rules: {
  receiver_mobile:[
    { validator:nb.pubfn.validator("mobile"), message: '手机号格式错误', trigger: 'blur' }
  ],
},
 */
 ...
 * @param {Object} obj
 * nb.pubfn.objectDeleteInvalid(obj);
 */
 ...
 * @param {Object} 	obj1
 * @param {Object} 	obj2
 * nb.pubfn.objectAssign(obj1, obj2);
 * nb.pubfn.objectAssign(obj1, obj2, true);
 * nb.pubfn.objectAssign(obj1, obj2, false);
 */
 ...
 * @param {Object} 	obj
 * let newObj = nb.pubfn.copyObject(obj);
 */
 ...
 * @param {Object} 	obj
 * let newObj = nb.pubfn.deepClone(obj);
 */
 ...
 * that.form1 = nb.pubfn.formAssign(defaultData,itemData);
 */
 ...
 * @example
 * let arr = nb.pubfn.arr_concat(arr1, arr2, "id");
 */
 ...
 * nb.pubfn.setData(dataObj, name, value);
 */
...
/**
 * 检测参数是否不为空 结果与 nb.pubfn.isNull 相反
 * nb.pubfn.isNotNull(value);
 */
 ...
 * 检测整个对象是否没有一个属性是空值,如果有空值,则返回首个是空值的属性名
 let nullKey = nb.pubfn.isNullOneByObject({ value1,value2,value3 });
 ...
 * @description 主要用于在一个对象数组中快速获取 id = 1 的对象
 ...
 * @description 主要用于在一个对象数组中快速获取 id = 1 的index
 * @param	{Array} list 数据源
 * @param	{String} key 键名
 * @param	{String} value 键值
 * @example
 * nb.pubfn.getListIndex(list, 'id', value);
 */
...
/**
 * 获取对象数组中某个元素的index和item,根据指定的键名和键值
 * @description 主要用于在一个对象数组中快速获取 id = 1 的index和item
 * @param	{Array} list 数据源
 * @param	{String} key 键名
 * @param	{String} value 键值
 * @example
 * nb.pubfn.getListItemIndex(list, 'id', value);
 */
 ...
 * 数组转对象 - 将对象数组转成json
 * @example
 * // 如 [{"id":"001","name":"name1"},{"id":"002","name":"name2"}]
 * // 转成 {"001":{"id":"001","name":"name1"},"002":{"id":"002","name":"name2"}}
 ...
 * // 如 [{"id":"001","name":"name1"},{"id":"002","name":"name2"}]
 * // 提取 id 字段转成 ["001","002"]
 * let idArr = nb.pubfn.arrayObjectGetArray(list, "id");
 */
 ...
 * @param {String} key 如果是对象数组，指定排序的键名
 * @param {String} order 排序方式 'asc' 升序(默认) 'desc' 降序
 ...
 */
pubfn.arraySort = function(arr, key, order = 'asc') {
	if (!arr || !Array.isArray(arr)) return [];
	const newArr = [...arr];
 ...
 * nb.pubfn.arrayIntersect([{id:1},{id:2}], [{id:2},{id:3}], 'id'); // [{id:2}]
 */
pubfn.arrayIntersect = function(arr1, arr2, key) {
	if (!arr1 || !Array.isArray(arr1)) return [];
...
/**
 * 数组并集（返回arr1和arr2的所有元素，去重）
 * @param {Array} arr1 数组1
 * @param {Array} arr2 数组2
 * @param {String} key 如果是对象数组，指定用于去重的键名
 * @return {Array} 并集数组
 * @example
 * nb.pubfn.arrayUnion([1,2,3], [2,3,4]); // [1,2,3,4]
 * nb.pubfn.arrayUnion([{id:1},{id:2}], [{id:2},{id:3}], 'id'); // [{id:1},{id:2},{id:3}]
 */
pubfn.arrayUnion = function(arr1, arr2, key) {
	if (!arr1) arr1 = [];
	if (!arr2) arr2 = [];
	const combined = [...arr1, ...arr2];
	return pubfn.arrayUnique(combined, key);
};
 ...
 * nb.pubfn.random(6);
 * nb.pubfn.random(6, "abcdefghijklmnopqrstuvwxyz0123456789");
 * nb.pubfn.random(1,"12",["1","2"]);
 */
 ...
 * nb.pubfn.stringIdToNumberId(uid,8);
 */
 ...
 */
pubfn.trimStart = function(str) {
	if (typeof str !== 'string') return str;
	return str.trimStart ? str.trimStart() : str.replace(/^\s+/, '');
};
 ...
 */
pubfn.trimEnd = function(str) {
	if (typeof str !== 'string') return str;
	return str.trimEnd ? str.trimEnd() : str.replace(/\s+$/, '');
};
 ...
 */
pubfn.padStart = function(str, targetLength, padString = ' ') {
	str = String(str);
 ...
 * nb.pubfn.padEnd('123', 5, '0'); // '12300'
 */
pubfn.padEnd = function(str, targetLength, padString = ' ') {
	str = String(str);
 ...
 * nb.pubfn.checkArrayIntersection(arr1, arr2);
 */
 ...
 * nb.pubfn.checkDataExpText(data, expText);
 */
...
/**
 * 判断变量是否是函数
 * nb.pubfn.isFunction(value);
 */
pubfn.isFunction = function(value) {
	return Object.prototype.toString.call(value) === "[object Function]" ? true : false;
};
/**
 * 判断变量是否是字符串
 * nb.pubfn.isString(value);
 */
pubfn.isString = function(value) {
	return Object.prototype.toString.call(value) === "[object String]" ? true : false;
};
/**
 * 判断变量是否是数字
 * nb.pubfn.isNumber(value);
 */
pubfn.isNumber = function(value) {
	return Object.prototype.toString.call(value) === "[object Number]" && !isNaN(value) ? true : false;
};
/**
 * 判断变量是否是布尔值
 * nb.pubfn.isBoolean(value);
 */
pubfn.isBoolean = function(value) {
	return Object.prototype.toString.call(value) === "[object Boolean]" ? true : false;
};
/**
 * 判断变量是否是日期对象
 * nb.pubfn.isDate(value);
 */
pubfn.isDate = function(value) {
	return Object.prototype.toString.call(value) === "[object Date]" ? true : false;
};
/**
 * 判断变量是否是正则表达式
 * nb.pubfn.isRegExp(value);
 */
pubfn.isRegExp = function(value) {
	return Object.prototype.toString.call(value) === "[object RegExp]" ? true : false;
};
/**
 * 判断变量是否是Promise
 * nb.pubfn.isPromise(value);
 */
pubfn.isPromise = function(value) {
	return value instanceof Promise || (value && typeof value.then === 'function' && typeof value.catch === 'function');
};
 ...
 * @example
 * nb.pubfn.calcFreights(freightItem, weight);
 */
 ...
 * let freightRes = nb.pubfn.calcFreightDetail(freightItem, weight);
 */
 ...
 * nb.pubfn.getNewObject(obj, keys);
 */
 ...
 * nb.pubfn.deleteObjectKeys(data, deleteKeys);
 */
 ...
 * @example
 * // { id:"id", parentId:"parentId", children:"children", deleteParentId:true }
 * nb.pubfn.arrayToTree(treeData, treeProps);
 * @deprecated 推荐使用 nb.pubfn.tree.arrayToTree
 */
pubfn.arrayToTree = pubfn.tree.arrayToTree;
/**
 * 树形结构转数组结构
 * @param {Array} treeData  数据源
 * @param {Object} treeProps 树结构配置
 * @example
 * // { id:"id", parentId:"parentId", children:"children", deleteChildren:true }
 * nb.pubfn.treeToArray(treeData, treeProps);
 * @deprecated 推荐使用 nb.pubfn.tree.treeToArray
 */
pubfn.treeToArray = pubfn.tree.treeToArray;

/**
 * 通配符匹配
 * @param {String} text  被匹配的文本
 * @param {String} expText 通配符规则
 * nb.pubfn.wildcardTestOne(text, expText);
 */
 ...
 * nb.pubfn.wildcardTest(text, expText);
 */
 ...
 * nb.pubfn.regExpTestOne(text, expText);
 */
 ...
 * nb.pubfn.regExpTest(text, regExp);
 */
...
/**
 * 生成UUID（v4版本）
 * 使用 crypto.randomUUID()（Node.js 和现代浏览器都支持）
 * @return {String} UUID字符串
 * @example
 * nb.pubfn.uuid(); // 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'
 */
pubfn.uuid = function() {
	// crypto.randomUUID() 在 Node.js 和现代浏览器中都可用
	if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
		return crypto.randomUUID();
	}
	// 兜底方案：手写实现（用于不支持 crypto.randomUUID 的旧环境）
...
/**
 * 生成短UUID（8位）
 * @return {String} 短UUID字符串
 * @example
 * nb.pubfn.shortUuid(); // 'a1b2c3d4'
 */
pubfn.shortUuid = function() {
	return pubfn.random(8, 'abcdefghijklmnopqrstuvwxyz0123456789');
};
 ...
 */
pubfn.encodeBase64 = function(str) {
 ...
 */
pubfn.decodeBase64 = function(str) {
 ...
 * @param {String} thousandsSep 千分位符号，默认','
 * @return {String} 格式化后的金额字符串
 * @example
 * nb.pubfn.formatMoney(1234567.89); // '1,234,567.89'
 * nb.pubfn.formatMoney(1234567.89, 0); // '1,234,568'
 */
pubfn.formatMoney = function(amount, decimals = 2, decPoint = '.', thousandsSep = ',') {
	if (pubfn.isNull(amount)) return '';
	...
	const decPart = parts[1] ? decPoint + parts[1] : '';
 ...
 * @example
 * nb.pubfn.parseQueryString('http://example.com?a=1&b=2'); // {a:'1',b:'2'}
 */
pubfn.parseQueryString = function(url) {
 ...
 */
pubfn.getUrlParam = function(name, url) {
	const params = pubfn.parseQueryString(url);
 ...
 *   () => fetch('/api/3')
 * ];
 * const results = await nb.pubfn.batchRun(tasks, 2);
 */
pubfn.batchRun = async function(tasks, limit = 5) {
 ...
 */
pubfn.uncapitalize = function(str) {
	if (typeof str !== 'string' || str.length === 0) return str;
 ...
 */
pubfn.truncate = function(str, length, suffix = '...') {
	if (typeof str !== 'string') return str;
 ...
 */
pubfn.unescapeHtml = function(str) {
	if (typeof str !== 'string') return str;
	const map = {
		'&amp;': '&',
		'&lt;': '<',
 ...
 * nb.pubfn.parseJSON('invalid', {}); // {}
 */
pubfn.parseJSON = function(str, defaultValue = null) {
	try {
 ...
 */
pubfn.stringifyJSON = function(value, defaultValue = '') {
	try {
 ...
 * @example
 * nb.pubfn.compareVersion('1.2.3', '1.2.4'); // -1
 * nb.pubfn.compareVersion('2.0.0', '1.9.9'); // 1
 */
pubfn.compareVersion = function(v1, v2) {
 ...
 */
pubfn.merge = function(target, ...sources) {
	if (!target) target = {};
	for (const source of sources) {
		if (!source || typeof source !== 'object') continue;
		for (const key in source) {
			if (source.hasOwnProperty(key)) {
				if (pubfn.isObject(source[key]) && pubfn.isObject(target[key])) {
					target[key] = pubfn.merge({}, target[key], source[key]);
				} else if (pubfn.isArray(source[key])) {
 ...
 */
pubfn.isEqual = function(a, b) {
	if (a === b) return true;
	if (typeof a !== typeof b) return false;
	if (a === null || b === null) return a === b;
	
	const typeA = pubfn.getType(a);
	const typeB = pubfn.getType(b);
	if (typeA !== typeB) return false;
	
	if (typeA === 'array') {
		if (a.length !== b.length) return false;
		for (let i = 0; i < a.length; i++) {
			if (!pubfn.isEqual(a[i], b[i])) return false;
		}
		return true;
	}
	...
	if (typeA === 'object') {
		...
		for (const key of keysA) {
			if (!b.hasOwnProperty(key) || !pubfn.isEqual(a[key], b[key])) return false;
		}
		return true;
	}
 ...
 * @example
 * nb.pubfn.groupBy([{type:'a',v:1},{type:'b',v:2},{type:'a',v:3}], 'type');
 * // {a:[{type:'a',v:1},{type:'a',v:3}], b:[{type:'b',v:2}]}
 */
pubfn.groupBy = function(arr, key) {
	if (!arr || !Array.isArray(arr)) return {};
 ...
 * 数组求和
 * @param {Array} arr 数组
 * @param {String} key 如果是对象数组，指定求和的键名
 * @return {Number} 求和结果
 * @example
 * nb.pubfn.sum([1, 2, 3]); // 6
 * nb.pubfn.sum([{v:1},{v:2}], 'v'); // 3
 */
pubfn.sum = function(arr, key) {
 ...
 * 数组求平均值
 * @param {Array} arr 数组
 * @param {String} key 如果是对象数组，指定求平均的键名
 * @return {Number} 平均值
 * @example
 * nb.pubfn.average([1, 2, 3]); // 2
 * nb.pubfn.average([{v:1},{v:2},{v:3}], 'v'); // 2
 */
pubfn.average = function(arr, key) {
	if (!arr || !Array.isArray(arr) || arr.length === 0) return 0;
	return pubfn.sum(arr, key) / arr.length;
};

/**
 * 数组求最大值
 * @param {Array} arr 数组
 * @param {String} key 如果是对象数组，指定比较的键名
 * @return {Number|Object} 最大值或最大值对象
 * @example
 * nb.pubfn.max([1, 2, 3]); // 3
 * nb.pubfn.max([{v:1},{v:3},{v:2}], 'v'); // {v:3}
 */
pubfn.max = function(arr, key) {
 ...
 * @param {Array} arr 数组
 * @param {String} key 如果是对象数组，指定比较的键名
 * @return {Number|Object} 最小值或最小值对象
 * @example
 * nb.pubfn.min([1, 2, 3]); // 1
 * nb.pubfn.min([{v:1},{v:3},{v:2}], 'v'); // {v:1}
 */
pubfn.min = function(arr, key) {
	if (!arr || !Array.isArray(arr) || arr.length === 0) return undefined;
	if (key) {
 ...
 * @example
 * nb.pubfn.shuffle([1, 2, 3, 4, 5]); // [3, 1, 5, 2, 4] (随机顺序)
 */
pubfn.shuffle = function(arr) {
	if (!arr || !Array.isArray(arr)) return [];
	const result = [...arr];
 ...
 * @return {Any} 随机元素
 * @example
 * nb.pubfn.sample([1, 2, 3, 4, 5]); // 随机返回一个元素
 */
pubfn.sample = function(arr) {
	if (!arr || !Array.isArray(arr) || arr.length === 0) return undefined;
 ...
 * @return {Array} 随机元素数组
 * @example
 * nb.pubfn.sampleSize([1, 2, 3, 4, 5], 3); // 随机返回3个元素
 */
pubfn.sampleSize = function(arr, n) {
	if (!arr || !Array.isArray(arr)) return [];
	const shuffled = pubfn.shuffle(arr);
 ...
 * nb.pubfn.string2Number(obj, option);
 */
 ...
 * nb.pubfn.checkFileSuffix(url,["png", "jpg", "jpeg", "gif", "bmp", "svg"]);
 */
 ...
 * nb.pubfn.percentageFilter(money);
 */
 ...
 * precision	数值精度
 * nb.pubfn.calcSize(length,["B","KB","MB","GB"],1024,3);
 */
 ...
 * 代码示例
 * let newArray = nb.pubfn.splitArray(array, 2);
 */
 ...
 * 代码示例
 * let newObj = nb.pubfn.objectKeySort(obj);
 */
 ...
 * 将blob转换为file
 * @example
 * nb.pubfn.blobToFile(blob, fileName);
 */
 ...
 * 文件转base64（浏览器端）
 * @example
 * nb.pubfn.fileToBase64({ file }).then(base64 => { ... });
 */
 ...
 * base64转文件（浏览器端）
 * @example
 * nb.pubfn.base64ToFile({ base64 }).then(file => { ... });
 */
pubfn.base64ToFile = function(obj = {}) {
	...
	return new Promise(function(resolve, reject) {
		...
		let file = pubfn.blobToFile(blob, filePath);
