...
import { Tag, Image, Button, Form, TreeSelect } from 'antd';
...
FIELD_TYPE_REGISTRY['tree-select'] = {
	...
	form: (config) => {
		...
		// 由于 ProFormTreeSelect 内部使用了废弃属性，直接使用 TreeSelect + Form.Item
	...
	form: (config) => {
		...
		const IconPickerField = () => {
			...
			const handleSelect = (iconKey) => {
				setLocalValue(iconKey);
				// 使用 setTimeout 避免同步更新导致的循环
				setTimeout(() => {
					form?.setFieldsValue?.({ [props.name]: iconKey });
				}, 0);
				setOpen(false);
			};
			
			const IconComponent = localValue ? renderIcon(localValue, { style: { fontSize: 20 } }) : null;
