 ...
 * 
 * 自动支持连表查询：
 * - 如果 config.query.foreignDB 配置了副表，getList 和 getDetail 都会自动连表
 * - getDetail 会使用相同的 foreignDB 配置
 */
export function createReadOnlyActions(config, options = {}) {
	...
	return {
		...
		getDetail: wrapAction(`${prefix}Get${resource}Detail`, async (params) => {
			// 支持直接传 id 字符串或 { id } 对象
			const id = nb.pubfn.isString(params) ? params : params?.id;
			if (!id) {
				return { success: false, error: 'ID is required' };
			}
			// 自动使用配置中的 foreignDB 进行连表查询
			return await dao.getDetail(id, {
				foreignDB: defaultForeignDB,
			});
		}, { skipLog: true }),
