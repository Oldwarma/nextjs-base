...
export function createCrudActions(config, options = {}) {
	...
	const resource = pascalCase(config.collectionName || 'resource');
	...
	return {
		...
		// 获取详情
		getDetail: wrapAction(`${prefix}Get${resource}Detail`, async (id, ctx) => {
		...
		// 更新
		update: wrapAction(`${prefix}Update${resource}`, async (id, data, ctx) => {
			return await dao.update(id, data);
		}),

		// 删除
		delete: wrapAction(`${prefix}Delete${resource}`, async (id, ctx) => {
