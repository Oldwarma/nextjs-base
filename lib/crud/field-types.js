...
export const FIELD_TYPE_REGISTRY = {
	...
	image: {
		...
		form: (config) => {
			const { PlusOutlined, UploadOutlined } = require('@ant-design/icons');
			const { Upload, Form, Button } = require('antd');
			const { App } = require('antd');
			const React = require('react');
			
			const listType = config.form?.listType || 'picture-card';
			const accept = config.form?.accept || 'image/*';
			const maxSize = config.form?.maxSize || 10; // MB
			const maxWidth = config.form?.width;
			const maxHeight = config.form?.height;
			...
			const SingleImageUpload = ({ value, onChange }) => {
				...
				const [fileList, setFileList] = React.useState(() => {
					if (typeof value === 'string' && value) {
						return [{ uid: '-1', url: value, status: 'done', name: 'image' }];
					}
					return [];
				});
				...
				React.useEffect(() => {
					if (typeof value === 'string' && value) {
						setFileList([{ uid: '-1', url: value, status: 'done', name: 'image' }]);
					} else if (!value) {
						setFileList([]);
					}
				}, [value]);
						...
						return new Promise((resolve, reject) => {
							...
							img.onload = () => {
								...
								if (maxWidth && img.width > maxWidth) {
									message.error(`Image width must be less than ${maxWidth}px!`);
									reject(Upload.LIST_IGNORE);
									return;
								}
								if (maxHeight && img.height > maxHeight) {
									message.error(`Image height must be less than ${maxHeight}px!`);
									reject(Upload.LIST_IGNORE);
									return;
								}
								resolve(file);
				...
				const handleChange = ({ fileList: newFileList }) => {
					...
					if (newFileList.length === 0) {
						...
						if (file.status === 'done') {
							const url = file.response?.url || file.url || (file.originFileObj ? URL.createObjectURL(file.originFileObj) : null);
							onChange?.(url);
						}
					}
				};
	...
	images: {
		...
		form: (config) => {
			const { PlusOutlined, HolderOutlined, DeleteOutlined, EyeOutlined } = require('@ant-design/icons');
			const { Upload, Form, Modal } = require('antd');
			const { App } = require('antd');
			const React = require('react');
			const { DndContext, closestCenter, KeyboardSensor, PointerSensor, useSensor, useSensors } = require('@dnd-kit/core');
			const { arrayMove, SortableContext, sortableKeyboardCoordinates, useSortable, rectSortingStrategy } = require('@dnd-kit/sortable');
			const { CSS } = require('@dnd-kit/utilities');
			
			const max = config.form?.max || 9;
			const accept = config.form?.accept || 'image/*';
			const maxSize = config.form?.maxSize || 10; // MB
			const sortable = config.form?.sortable !== false; // 默认支持排序
						...
						>
							{sortable && (
								<span
									{...attributes}
									{...listeners}
									style={{ color: '#fff', cursor: 'grab', fontSize: 16 }}
								>
									<HolderOutlined />
								</span>
							)}
	...
	file: {
		...
		form: (config) => {
			const { UploadOutlined, InboxOutlined, HolderOutlined, DeleteOutlined, PaperClipOutlined } = require('@ant-design/icons');
			const { Upload, Form, Button } = require('antd');
			const { App } = require('antd');
			const React = require('react');
			const { DndContext, closestCenter, KeyboardSensor, PointerSensor, useSensor, useSensors } = require('@dnd-kit/core');
			const { arrayMove, SortableContext, sortableKeyboardCoordinates, useSortable, verticalListSortingStrategy } = require('@dnd-kit/sortable');
			const { CSS } = require('@dnd-kit/utilities');
			
			const max = config.form?.max || 5;
			const accept = config.form?.accept;
			const maxSize = config.form?.maxSize || 10; // MB
			const useDragger = config.form?.dragger || false;
			const sortable = config.form?.sortable !== false; // 默认支持排序
			...
			// 文件上传组件
			const FileUploadComponent = ({ value, onChange }) => {
				const { message } = App.useApp();
				const [fileList, setFileList] = React.useState([]);
					...
					if (Array.isArray(value) && value.length > 0) {
						const newFileList = value.map((item, index) => {
							if (typeof item === 'string') {
								return { uid: `existing-${index}-${Date.now()}`, url: item, status: 'done', name: item.split('/').pop() || `file-${index}` };
							}
							return { ...item, uid: item.uid || `existing-${index}-${Date.now()}`, status: 'done' };
						});
						setFileList(newFileList);
					} else if (!value || value.length === 0) {
						setFileList([]);
				...
				const customRequest = ({ file, onSuccess, onError }) => {
					...
					setFileList(prevList => {
						const newFileList = [...prevList, newFile];
						// 使用 setTimeout 确保状态更新后再通知
						setTimeout(() => {
					...
					onSuccess({ url: newFile.url });
				};
				
				// 处理默认 Upload 组件的 onChange（用于删除等操作）
				const handleDefaultChange = ({ fileList: newFileList }) => {
					// 只处理删除操作
					setFileList(newFileList);
					notifyChange(newFileList);
				};
				...
				// 拖拽上传区域样式（官方样式）
				// 参考: https://5x.ant.design/components/upload-cn#upload-demo-drag
				if (useDragger) {
					return (
						<div>
							{fileList.length < max && (
								...
								>
									...
									>
										<div style={{ textAlign: 'center', padding: '20px 0' }}>
											...
											<p style={{ margin: '8px 0 4px', color: 'rgba(0, 0, 0, 0.85)', fontSize: 16 }}>
												{description}
											</p>
							...
							{sortable ? renderSortableFileList() : renderFileList()}
						</div>
					);
				}
					...
					return (
						<div>
							<Upload
								...
								{...config.form?.fieldProps}
							>
								<Button icon={<UploadOutlined />} disabled={fileList.length >= max}>
									Click to Upload
								</Button>
							</Upload>
	...
	avatar: {
		...
		form: (config) => {
			...
			const { Upload, Form } = require('antd');
			const { App } = require('antd');
			const React = require('react');
			
			const accept = config.form?.accept || 'image/png,image/jpeg,image/gif';
			const maxSize = config.form?.maxSize || 2; // MB
			
			// 头像上传组件 - 复用单图上传逻辑，使用圆形样式
			const AvatarUpload = ({ value, onChange }) => {
				const { message } = App.useApp();
				const [fileList, setFileList] = React.useState(() => {
					if (typeof value === 'string' && value) {
						return [{ uid: '-1', url: value, status: 'done', name: 'avatar' }];
					}
					return [];
				});
				...
				React.useEffect(() => {
					if (typeof value === 'string' && value) {
						setFileList([{ uid: '-1', url: value, status: 'done', name: 'avatar' }]);
					} else if (!value) {
						setFileList([]);
					}
				}, [value]);
 ...
 * 
 * 参考:
 * - vk-unicloud: https://vkdoc.fsq.pub/admin/3/form.html#props
 * 
