...
export const FIELD_TYPE_REGISTRY = {
	...
	image: {
		...
		form: (config) => {
			...
			// 转换初始值：将 URL 字符串转换为 fileList 格式
			const convertValue = (value) => {
				if (!value) return [];
				if (Array.isArray(value)) {
					return value.map((item, index) => {
						if (typeof item === 'string') {
							return { uid: `${index}`, url: item, status: 'done', name: item.split('/').pop() || `image-${index}` };
						}
						return item;
					});
				}
				if (typeof value === 'string') {
					return [{ uid: '0', url: value, status: 'done', name: value.split('/').pop() || 'image' }];
				}
				return [];
			};
		...
		form: (config) => {
			...
			const convertValue = (value) => {
				...
				if (Array.isArray(value)) {
					return value.map((item, index) => {
						if (typeof item === 'string') {
							return { uid: `${index}`, url: item, status: 'done', name: item.split('/').pop() || `file-${index}` };
						}
						return item;
					});
				}
				if (typeof value === 'string') {
					return [{ uid: '0', url: value, status: 'done', name: value.split('/').pop() || 'file' }];
				}
				return [];
			};
	...
	avatar: {
		...
		form: (config) => {
			...
			const convertValue = (value) => {
				if (!value) return [];
				if (typeof value === 'string') {
					return [{ uid: '0', url: value, status: 'done', name: 'avatar' }];
				}
				if (Array.isArray(value) && value.length > 0) {
					const item = value[0];
					if (typeof item === 'string') {
						return [{ uid: '0', url: item, status: 'done', name: 'avatar' }];
					}
					return [item];
				}
				return [];
