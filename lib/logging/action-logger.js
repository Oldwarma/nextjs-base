...
function getSummary(data) {
	...
	if (typeof data === 'object') {
		...
		return keys.length <= 3 ? data : { ...preview, _keys: `${keys.length} keys` };
	}
 ...
 * 记录 Server Action 日志
 */
export function logAction(actionName, category, startTime, requestTime, params, result, isError = false) {
...
export function withActionLog(actionName, category, actionFn) {
	return async function (...args) {
		...
		try {
			...
			logAction(actionName, category, startTime, requestTime, args.length === 1 ? args[0] : args, errorResult, true);
