...
export async function createUsageLog(userId, usageData) {
	...
	const insertResult = await usageLogsCollection.insertOne(usageLog);
	const usageId = insertResult.insertedId;
	...
	if (!deductResult.success) {
		// 如果扣除失败，删除刚创建的记录
		await usageLogsCollection.deleteOne({ _id: usageId });
...
export async function updateUsageLog(usageId, updateData) {
	...
	const updateResult = await usageLogsCollection.updateOne({ _id: usageId }, { $set: update });
...
export async function getUserUsageLogs(userId, options = {}) {
	...
	return usageLogsCollection.findWithPagination({
		query,
		pageIndex,
		pageSize,
		sort: { createdAt: -1 },
...
export async function getUserUsageStatistics(userId, options = {}) {
	...
	// 获取所有记录进行统计
	const logs = await usageLogsCollection.find(query);
...
export async function getUsageLogById(usageId) {
	const usageLogsCollection = await getCollection('usage_logs');
	const usageLog = await usageLogsCollection.findOne({ _id: usageId });
...
export async function deleteUsageLog(usageId) {
	...
	const result = await usageLogsCollection.deleteOne({ _id: usageId });
